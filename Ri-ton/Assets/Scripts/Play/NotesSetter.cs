...
public class NotesSetter : MonoBehaviour
{
    ...
    private void SetLongNotes()
    {
        ...
        foreach (MusicDTO.Note note in validNoteList)
        {
            if ((note.lane == 1) || (note.lane == 2))
            {
                SetLongNotesFunc(note, ref evenNumberNongNoteInfo, ref usedEvenNumberNoteNum, true);
            }
            else
            {
                SetLongNotesFunc(note, ref oddNumberNongNoteInfo, ref usedOddNumberNoteNum, false);
            }
        }
    }

    private void SetLongNotesFunc(MusicDTO.Note note, ref LongNoteInfo info, ref int usedNoteNum, bool isEvenNumber)
    {
        int num = 0;
        ...
        for (int i = 0; i < info.objList.Count; i++)
        {
            ...
            if (num > usedNoteNum)
            {
                ...
                {
                    Color color = (isEvenNumber) ? evenNumberLongNoteColor.holdingColor : oddNumberLongNoteColor.holdingColor;
